...
subset <- select(astronauts, name, nationality, sex) %>%
  distinct() 
...
female <- filter(subset, sex =="female")
...
p2 <- ggplot(data, aes(x=reorder(nationality,-count), y=count))+
  geom_bar(stat="identity", fill="#33cccc")+
  geom_text(aes(label=count), hjust=-.4, color="white", fontface="bold", size=5.5)+
  coord_flip()+
  labs(
    y="",
    x="",
    title = "Number of Female Astronauts by Country, 1961-2019"
  )+
  scale_y_continuous(breaks = seq(from = 0, to = 50, by = 10))+
  theme(
    axis.text = element_text(colour = "white",size=15),
    plot.title = element_text(color="white", hjust=.3, size=20),
